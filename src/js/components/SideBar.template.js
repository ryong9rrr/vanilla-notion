import { isValidTypeOfObject } from "../core/validate.js";
import { TYPE_DOCUMENT } from "../types/index.js";
...
const makeListHtml = (documents, depth = 1) => {
  if (documents.length === 0) {
    return `
      <ul class="nested sidebar-document-list-last" style="padding-left:${
        15 * depth
      }px">
        하위 페이지가 없어요.
      </ul>`;
  ...
  return documents
    .map((document) => {
      const { id, title, documents } = (document = isValidTypeOfObject(
        document,
        TYPE_DOCUMENT
      ));
      ...
      return `
          <ul class="${depth > 1 ? "nested" : ""}">
            <li class="${
              CLASS_NAME.list
            } sidebar-component" style="padding-left:${
        15 * depth
              ...
              } sidebar-add-remove-button">삭제</button>
              <button class="${
                CLASS_NAME.addButton
              } sidebar-add-remove-button">추가</button>
            </li>
...
export const template = ({ username, documents }) => {
  return `
