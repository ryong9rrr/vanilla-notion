...
const request = async (url, options = {}) => {
  try {
    ...
    console.error(
      `API 호출에 문제가 있는 것 같아요. 에러 메시지: ${e.message}`
    );
    throw new Error("API 호출 오류");
  }
};
...
export const postNewDocument = async (requestBodyObj) => {
...
export const editDocument = async (documentId, requestBodyObj) => {
  if (
    requestBodyObj.title === undefined ||
    requestBodyObj.content === undefined
  )
